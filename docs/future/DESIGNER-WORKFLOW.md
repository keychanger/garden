...
> **This design has shipped** (2026-07): the `designer` workflow is in the
> registry, `garden designer publish` exists, and the skill is bundled.
> The authoritative description is `WORKFLOWS.md` § designer plus
...
> `isPublishablePath`; scope violations park as `designer-scope`; the
...
Design record for `designer`, a shipped garden workflow whose unit of
...
fuse design and build into one worker on one model, a designer splits them: the
designer designs, an implementing worker builds, a strong reviewer checks. Each
...
- **Phase 2** (crew-parameterized assisted handoff, artifact-type awareness):
  ...
  on approval the designer publishes then spawns a default-workflow builder
  ...
  open. Authoritative behavior: WORKFLOWS.md § "Designer workflow".
...
- **From this doc's prior self:** the four-phase design pipeline, the
  ...
  artifacts-directory-as-resume-oracle, no-reviewer-on-the-designer-branch.
- **From [`PLAN-WORKFLOW.md`](PLAN-WORKFLOW.md) (its sibling, not its rival):**
  ...
  execution; designer converts intent → a **design doc** for human approval.
...
- **From the design conversation:** that the ROI concentrates in three specific
  ...
  designer and the builder are the same strong model with no gate — so the
...
Designers invert that posture. Their explicit job is to think out loud, propose
...
sends back for another round. When the operator approves, the designer's last
...
2. **Model.** Once design and build are separate *seats*, they no longer have
   ...
   — is the whole point. A designer run is the front of a **model-diverse
...
## The core idea: design is a role, designer is the lifecycle
...
|---|---|---|
| **Workflow** | The *lifecycle* — how many gates, does it merge code, who dispatches next. | `designer` (this doc): frame → options → converge → publish → handoff. |
...
A designer run therefore has **three seats**, and each resolves independently:

- **Designer** — the designer worker itself. Judgment-heavy, so it defaults to
...
too — as its own next step; the designer worker is simply a `designer` role
...
knobs a designer run exposes are **its own model/harness** (the designer seat)
...
A designer session walks through four phases inside the `working` state. Phase
...
crashed designer resumes by reading which files exist.

```
[1 frame]           Designer reads the seed, scans the repo, and writes a
                    problem-framing doc: what is the operator trying to
                    accomplish, what constraints exist, what is out of scope.
                    Output: <worktree>/.garden/designer/<id>/framing.md
       v
[2 options]         Designer drafts 2-3 distinct approaches with tradeoffs.
       ...
       v
[3 converge]        Operator responds in chat. Designer incorporates answers,
       ...
       v
[4 publish]         On operator approval, designer moves the artifact to its
...
Phase 3 (converge) is the human gate and **loops** — the designer re-enters it
...
stays *off* for the entire run; the designer only re-engages when the operator
types.
...
|---|---|---|---|---|
| 1 frame | designer prompt | seed (file or text) | framing.md | no |
...
A designer run exposes exactly two model choices, mapping onto the three seats:

1. **The designer seat = the designer worker's own harness/model.** Resolves
   like any worker: `--harness` / `--model` / project default, with a
   **designer-workflow default of Opus** (design is judgment-heavy). So `garden
   workers new <p> --workflow designer --harness codex` gives you a Codex
   designer; the default gives you Opus.

2. **The builder + reviewer seats = a handoff crew.** The designer carries a
   ...
   you configure expands the flip-set for free — no designer code changes.

"Flip Sonnet or Codex freely" is therefore two independent dials the operator
already understands: **who designs** (the designer's own model) and **which
...
safe precisely because its code is reviewed by a strong reviewer. Designer does
...
harness*, breaking the single-model monoculture. This is the designer realization
...
Is a designer a new workflow variant (alongside default / trellis / grow), or a
...
piece of awkwardness — designers short-circuit the review/merge state machine —
and the trellis precedent shows it is tractable: trellis already overrides
reviewer prompts, verdict vocab, and model pinning. Designer extends the
override pattern with a workflow flag `skipsReviewMerge: true` that the poller
honors: on `.garden-done`, the branch still merges (designer artifacts are real
...
keep: designer genuinely needs three things a preset cannot provide — the
...
multi-designer collaboration on one doc, a designer watching several worker
branches, or a designer whose deliverable is a beads graph (which belongs to
plan, not designer).

### Workflow definition

A new file `src/dashboard/workflows/designer.ts`:
...
export const designerWorkflow: WorkflowDefinition = {
  ...
  stateHandlers: {
    working: handleWorkingDesigner,       // honors .garden-awaiting-input
  ...
  reviewerModel: undefined, // no reviewer on the designer branch
  autoContinue: false,      // operator-driven; no post-turn re-prompt
  skipsReviewMerge: true,   // merge happens, but no review
};
```

`designerValidTransitions` drops `reviewing`, `resolving`, and `failing` — they
...
heavily action-biased. Designers need the inverse. Implementation:
...
`rules-designer.md`. Key inversions:
...
- **Write artifacts, not code.** Edits to `src/`, tests, configs, or build
  ...
  `<worktree>/.garden/designer/<id>/` plus, at publish time, the artifact's
  final location.
...
enforces the boundary: it inspects the diff and refuses to merge a designer
...
convention plan uses. The directory is the designer's working memory across
...
Designer relies on the `.garden-awaiting-input` sentinel shared with plan. (If
plan ships first, designer inherits it; if designer ships first, it carries the
...
The designer holds the sentinel by default during Phases 2 and 3 and captures
...
the designer recognizes ("approve", "ship it", "publish") or by `garden designer
publish <worker>`.
...
The natural endpoint of a designer run is: the operator approves, and an
implementing worker starts building — on the crew the operator chose.

### Phase 1: operator-driven handoff

The designer's `summary.md` and `handoff-brief.md` include a suggested handoff
...
plumbing, prove that a designer-authored artifact + a `--crew` choice produces a
...
The designer invokes the existing `handoff` skill at the end of Phase 4, which
...
```bash
garden designer publish [<worker>] --handoff <workflow> --handoff-crew <name>
```
...
`entry.crew` on the child, applied live by `resolveReviewRole`). Designer must
...
```bash
# Direct
garden workers new <project> --workflow designer --seed "Help me think about notification levels..."
garden workers new <project> --workflow designer --seed-file path/to/idea.md

# Designer-seat model/harness (defaults to Opus / claude-code)
garden workers new <project> --workflow designer --harness codex
garden workers new <project> --workflow designer --model opus

# Via picker
# Opt-Shift-N opens the workflow picker; (b) row selects designer and prompts
...
```bash
garden designer publish [<worker>] [--handoff <workflow>] [--handoff-crew <name>] [--dry-run]
```

Self-resolves via `$GARDEN_WORKER` when invoked from inside the designer's pane.
...
confusing problem can be converted to a designer mid-run:

```bash
garden workers designer [<worker>] --seed "Stop building, help me think about this first"
```

This pauses the current task, captures state in a `framing.md` draft, and flips
the workflow to designer. Out of scope for Phase 1; flagged for future.

## Skill bundle

A new `designer` skill bundled into designer worktrees via
...
and when and how to invoke `garden designer publish` (including the handoff
crew). It lives at `src/dashboard/skills/designer/SKILL.md` and is copied into
`.claude/skills/designer/` at worktree creation.

## Relationship to plan and crews

- **Plan and designer are siblings, not rivals.** Plan converts intent into a
  **beads graph** for parallel execution (`bd create` + dependency edges);
  designer converts intent into a **design doc** for human approval. A future
  combined flow reads naturally: designer designs → operator approves → plan
  decomposes the artifact into beads → default/trellis/grow workers (each on
  their chosen crew) execute. Designer does not produce beads; if it wants to,
  that is plan's job, invoked at handoff.
- **Crews are shipped; designer consumes them.** The crew axis,
  `resolveReviewRole`, the member model, the reviewer-strong invariant, and the
  `⌥⇧C` picker all exist today. Designer's Phase 1/2 seats reuse them directly —
...
Before writing `designer.ts`, validate the spine with existing tools: run a
...
**Deliverable**: end-to-end designer run that produces a trellis doc on a strong
...
- `src/dashboard/workflows/designer.ts` — workflow definition
- `src/dashboard/designer-prompts.ts` — designer system prompt
- `src/dashboard/designer-publish.ts` — publish handler (artifact move +
  writeable-path enforcement + write `.garden-done`)
- `src/commands/designer.ts` — `publish` subcommand
- `src/dashboard/continue.ts` — honor `.garden-awaiting-input` (shared with plan)
- `src/dashboard/skills/designer/SKILL.md` + `skills.ts` bundling
- `rules-designer.md` + `src/rules.ts` workflow-aware preamble selection
- Designer-seat wiring: `--harness` / `--model` already flow through
  ...
  default
- `WORKFLOWS.md` / `CLAUDE.md` — register designer, add `(b)` picker row

Acceptance: `garden workers new garden --workflow designer --seed "Let's
...
**Deliverable**: one command kicks off a designer that, after approval,
...
- `garden workers designer` to convert an active default worker
...
- **Optional design-review fan-out** (the PLAN pattern, scaled to design): a
  cheap `garden designer review <artifact>` pass that surfaces ambiguity / gaps
...
- **Designers that write code.** The whole point is the cognitive-mode split.
  A designer editing `src/` is misbehaving; the merge handler refuses it.
- **Reviewer fan-out as a requirement.** Designer produces prose the operator
...
- **Auto-handoff without operator approval.** Phase 2's `--handoff` is gated by
  explicit publish approval. There is no path from "designer drafted an
...
- **Designer as the planner.** Beads decomposition is plan's job.
...
2. **Handoff-crew default: project crew, or explicit-only?** Defaulting to the
   project's current crew is least-surprise, but a designer run is exactly when
...
5. **Should the designer read the codebase by default?** Recommended: rely on
   the `rules-designer.md` preamble to encourage reading-before-drafting; add a
...
|---|---|---|
| Designer edits files outside allowed paths | Diff inspection in publish handler | Refuse to publish, alert, park in `failing` |
...
| Designer publishes without operator approval | rules preamble + publish command is the gate | The publish command cannot be invoked without the operator; if run from the pane, the operator sees and can interrupt |
| Designer drifts into building | rules-designer.md preamble | Operator interrupts; no automated detection in Phase 1 |
| Approved artifact underspecified → builder bounces | builder's normal review cycle | `garden workers designer` against the failing builder to revise the spec, then re-handoff |
...
- `src/dashboard/workflows/designer.ts` (~60 lines) — workflow definition
- `src/dashboard/designer-prompts.ts` (~200 lines) — designer prompt
- `src/dashboard/designer-publish.ts` (~140 lines) — publish handler +
  writeable-path enforcement + handoff-crew threading
- `src/commands/designer.ts` (~90 lines) — `publish` subcommand
- `src/dashboard/skills/designer/SKILL.md` (~150 lines) — design-posture skill
- `rules-designer.md` (~80 lines) — design-posture preamble fragment
- `docs/future/DESIGNER-WORKFLOW.md` (this file)

Modified files:

- `src/dashboard/workflows/index.ts` — register `designerWorkflow`
- `src/dashboard/workflows/types.ts` — add `designerValidTransitions`, optional
...
- `src/dashboard/skills.ts` — bundle `designer` skill
- `src/rules.ts` — workflow-aware preamble selection
- `src/cli.ts` — register `designer` command group; `designer` in `--workflow`
  help; `--handoff-crew` flag
- the workflow picker — add `(b)` row + crew composer row
- `WORKFLOWS.md` — add "Designer workflow" section after Phase 1 ships
- `CLAUDE.md` — list `designer` under "CLI surface" workflows
...
- [`PLAN-WORKFLOW.md`](PLAN-WORKFLOW.md) — the sibling that produces a beads
  ...
  artifacts-as-resume-oracle pattern; the future combined flow (designer designs
...
- [`WORKFLOWS.md`](../../WORKFLOWS.md) — the workflow axis designer joins; the
