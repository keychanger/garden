...
| [future/OPERATOR-UI.md](future/OPERATOR-UI.md) | speculative | Status-pane visual grammar (identity badges vs status colors, one shared renderer, workflow `renderRow`) plus a keybinding-driven config surface: project menu (⌥,), worker inspect/act menu (⌥i), spawn composer on ⌥⇧N, explicit `baseBranch`, per-worker crew. No code today. |
...
| [future/CREWS.md](future/CREWS.md) | speculative | Making Codex a first-class citizen: the **crew** axis (a named bundle of role→harness/model assignments, orthogonal to workflow) and the remaining Codex **worker path** (sandbox translation + macOS self-push footgun, interactive turn-end spike, AGENTS.md rules delivery, session-identity recovery, selection surface). Codex-as-reviewer already ships; the worker path does not. |
