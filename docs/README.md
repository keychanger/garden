...
- **Reference** — a map of behavior that is already implemented, written
  for an operator who must reason about it (e.g. a security posture and
  its residual risks). Not a contract in either direction: if it and the
  code disagree, whichever is wrong gets fixed.
...
| [future/AUTONOMY-PROGRAM.md](future/AUTONOMY-PROGRAM.md) | speculative | North-star map sequencing the autonomy future-docs into one program: the three ceilings (origination, model/harness utilization, open-loop), which existing doc owns each slice, the telemetry-and-scheduler-first ordering, two net-new pieces (Codex second harness, the Advisor pattern), and two forks (Fork 1, backlog substrate, resolved to beads 2026-07-24 — the delegation loop is being built board-side; Fork 2, default-worker model, still open). |
| [future/PLAN-WORKFLOW.md](future/PLAN-WORKFLOW.md) | speculative | The plan workflow: convert human-described features into a beads graph for autonomous execution. No code today — workers must not run `bd` commands or file beads. Beads won the substrate fork (2026-07-24); sequenced after the board-side delegation loop, whose planner phase is a scaled-down first cut of this design. Sibling of designer (designer designs the doc, plan decomposes it), not superseded by it. |
| [future/DESIGNER-WORKFLOW.md](future/DESIGNER-WORKFLOW.md) | design record (shipped) | The designer workflow: a design-mode authoring workflow whose output is a design artifact rather than a commit. **Shipped 2026-07** — authoritative description now in `WORKFLOWS.md`; the doc records the design and its as-built deviations. |
...
| [future/SPRIG.md](future/SPRIG.md) | speculative (partially superseded) | Sprig: latent self-improvement on idle quota — reviewer-enforced risk tiers, default-off surplus autoplant. Its flat-file backlog lost the substrate fork to beads (2026-07-24); the tiers, gate chain, and premature-done prerequisite survive and would re-target beads. No code today. |
...
| [future/CREWS.md](future/CREWS.md) | design record (shipped) | Making Codex a first-class citizen: the **crew** axis (a named bundle of role→harness/model/effort assignments, orthogonal to workflow) and the Codex **worker path**. **All six slices shipped** (2026-07-06 through 2026-07-19, incl. stored crews); residuals are live-verifying `codex resume`, Codex-aware draft detection, and trellis/grow harness selection. The doc is self-current and wins over older docs on shipped state. |
