...
| [future/OPERATOR-UI.md](future/OPERATOR-UI.md) | design record (shipped) | Status-pane visual grammar (identity badges vs status colors, one shared renderer) plus the keybinding-driven config surface: project menu (⌥,), worker menu (⌥i), spawn composer on ⌥⇧N, explicit `baseBranch`, per-worker crew. **Fully shipped 2026-07-15/16** — authoritative description now in `DESIGN.md`/`CLAUDE.md`. |
