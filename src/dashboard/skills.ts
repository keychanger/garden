...
3. Execute the handoff plan the operator approved. Publish first, then hand off — never the reverse: a failed publish must not leave a builder working from an unpublished design.
   ...
   - **Trellis builder**: this route is operator-run and two-step — the handoff mechanism spawns default-workflow workers only, and \`--trellis\` resolves a *named*, tagged trellis inside the project checkout's trellis dir (\`.garden/trellises/\` by default), never a \`docs/\` path. Before publishing, format the artifact with the trellis spine (see the \`trellis-author\` skill: the \`<!-- trellis: v1 -->\` tag in the first lines, the spec sentinel, the recommended sections). Then publish as usual and give the operator the two steps, run in the project checkout after the merge lands: \`cp docs/future/<name>.md .garden/trellises/<name>.md\` then \`garden workers new <project> --workflow trellis --trellis <name>\`. For a non-default builder crew, they set it on the project first (\`garden config <project> crew <name>\`) — per-spawn \`--crew\` is default-workflow only today, so it cannot ride the trellis plant.
