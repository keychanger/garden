// The publishable-path boundary for a botanist artifact. Kept in its own leaf
...
// lives in botanist-publish.ts and must never be pulled into the hook bundle.
import path from "node:path";

// The only place a botanist may publish. `.garden/` is git-excluded (never in a
// diff), so a botanist's sole committable output is a tracked doc under docs/.
...
export const BOTANIST_PUBLISH_ROOT = "docs/";
...
export function isPublishablePath(relPath: string): boolean {
  ...
  return normalized.startsWith(BOTANIST_PUBLISH_ROOT);
}
