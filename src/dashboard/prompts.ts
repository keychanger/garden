...
export function buildReviewPrompt(
  ...
  sections.push(
    ...
    "## Output Format",
    "",
    "If your verdict is FIXED or FAILED, immediately before the verdict line emit a",
    "fenced JSON block listing each distinct issue, in this exact format:",
    "",
    "```findings",
    "[",
    '  { "category": "stale-test-after-refactor",',
    '    "summary": "foo.test.ts still asserted the old return shape." }',
    "]",
    "```",
    "",
    "Rules for findings:",
    "- One entry per distinct issue you intervened on (FIXED) or flagged but could",
    "  not fix (FAILED). Skip duplicates of the same underlying problem.",
    "- `category` is a short kebab-case tag naming the CLASS of issue (not this",
    "  specific instance). Reuse the same tag verbatim across reviews for the same",
    "  kind of problem — tallying depends on exact matches. Allowed chars: a-z 0-9 -.",
    "- `summary` is one sentence describing this specific instance.",
    "- For CLEAN verdicts, do NOT emit a findings block.",
    "",
