...
const STALE_HOOK_MS = 15 * 60 * 1000;
...
export async function health(args: string[]): Promise<void> {
  ...
  for (const projectName of projectNames) {
    ...
    for (const entry of entries) {
      ...
      // than STALE_HOOK_MS without any hook fire is suspicious. Either Claude
      ...
      if (entry.claudeStatus === "working") {
        const lastHookAt = entry.lastHookAt ?? 0;
        const elapsed = Date.now() - lastHookAt;
        if (lastHookAt === 0 || elapsed > STALE_HOOK_MS) {
          const ageMin = lastHookAt === 0 ? "ever" : `${Math.round(elapsed / 60000)}min`;
          issues.push(
            `Worker ${entry.name}: claudeStatus=working but last hook ${ageMin} ago`,
          );
        }
      }

      // Dead-pane detection: if the pane PID is gone but claudeStatus is
      // not "exited", the tmux pane-died hook missed the death event.
      // With --fix, write claudeStatus="exited" to correct the registry.
      if (entry.claudeStatus !== "exited") {
        ...
        if (paneId) {
          ...
          if (pid) {
            ...
            if (!alive) {
              issues.push(
                `Worker ${entry.name}: pane PID ${pid} is dead but claudeStatus=${entry.claudeStatus ?? "unknown"}`,
              );
              if (fix) {
                try {
