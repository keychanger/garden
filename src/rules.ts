...
- When your task is complete, commit and push your branch — this applies to *every* file you created or changed (docs, AGENTS.md, scripts, configs), not just code. Writing a file to disk is not delivery; until it is pushed, the poller never sees it and your work is invisible. Before you end your turn, confirm \`git status\` is clean and \`git log @{u}..HEAD\` is empty — if either shows pending work, commit and push it. The poller will automatically rebase, review, and merge your changes into ${baseBranch}. After pushing, end your turn — garden's auto-continue prompt fires after the merge so multi-phase work keeps building without operator intervention. If that push completes the operator's *full* original request — every deliverable they asked for has landed and is verified, not just the current one — invoke the \`done\` skill (\`touch .garden-done\`) on the same turn so the merge transitions straight to \`done\` and skips a wasted round-trip. Stopping early is strictly preferred over inventing extra work; but "done" means you re-read the request and accounted for every deliverable, not that you feel finished — the operator will redirect if more is needed. If a deliverable has NOT landed and you cannot land it without a decision only the operator can make (a product-shape question, a spend or credentials approval, a choice the request gives you no basis to make), that is a third case — neither continue nor \`done\`: run \`garden blocked "<the decision you need>"\` (the \`blocked\` skill) and end your turn, which puts the question on your dashboard row and alerts the operator. Do NOT use \`done\` for it; \`done\` reads as "the task is complete" and hides what is actually waiting on them.
...
**Finish by rewriting the epic's plan label and stopping.** On success the epic moves to \`plan:ready\`; on ANY failure land \`plan:failed\` — never leave the epic parked at \`plan:planning\`. Never \`bd promote\` (the operator's promotion gesture in board owns that), and never delete or edit beads outside the epic you were seeded with. Then end your turn: you are done, and no merge or review follows.`;
  ...
  if (options?.designer) {
    ...
    // push when complete / invoke the done skill / auto-continue after merge"
