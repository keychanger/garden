...
**Auto-continue across phases.** After your branch is reviewed and merged, garden automatically sends a "please proceed" prompt to your pane so multi-phase work keeps building on the merged base without operator intervention. When you have completed *everything* the operator asked for, write a sentinel file at \`/tmp/garden-$GARDEN_PROJECT-$GARDEN_WORKER.done\` before ending your turn — its presence tells garden not to auto-continue on the next merge. (The path is in \`/tmp\` rather than \`~/.garden/sessions\` because that's the only directory outside your worktree the sandbox lets you write to.) If you receive an auto-continue prompt and there is genuinely nothing left to do, just say so briefly and end your turn; one wasted turn is fine, but silently stopping mid-task is not.`;
}
