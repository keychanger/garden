...
- When your task is complete, commit and push your branch — this applies to *every* file you created or changed (docs, CLAUDE.md, scripts, configs), not just code. Writing a file to disk is not delivery; until it is pushed, the poller never sees it and your work is invisible. Before you end your turn, confirm \`git status\` is clean and \`git log @{u}..HEAD\` is empty — if either shows pending work, commit and push it. The poller will automatically rebase, review, and merge your changes into ${baseBranch}. **Then decide whether the push you just made completes the operator's full original request, not just the current phase.** If yes, invoke the \`done\` skill (\`touch .garden-done\`) on the same turn — the merge will transition straight to \`done\` instead of waking you with a "please proceed" prompt you'd only answer with "I was already finished." If you're mid-phase, leave the sentinel alone.
