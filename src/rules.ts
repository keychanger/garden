...
- When your task is complete, commit and push your branch — this applies to *every* file you created or changed (docs, CLAUDE.md, scripts, configs), not just code. Writing a file to disk is not delivery; until it is pushed, the poller never sees it and your work is invisible. Before you end your turn, confirm \`git status\` is clean and \`git log @{u}..HEAD\` is empty — if either shows pending work, commit and push it. The poller will automatically rebase, review, and merge your changes into ${baseBranch}. **Default: end your turn after pushing — do not write the \`.garden-done\` sentinel.** Garden's auto-continue prompt is the intended way to advance between phases, so the sentinel is a narrow exception, not part of the standard end-of-turn checklist. The exception: if that push completes the operator's *full* original request (every phase, not just the current one), invoke the \`done\` skill (\`touch .garden-done\`) on the same turn so the merge transitions straight to \`done\` and skips a wasted round-trip. When uncertain whether anything remains, prefer writing the sentinel — stopping early is strictly preferred over inventing extra work, and the operator will redirect if more is needed.
...
**Auto-continue across phases.** After your branch is reviewed and merged, garden sends a merge-notification prompt to your pane so multi-phase work can keep building on the merged base without operator intervention. When the prompt arrives, treat **invoking \`done\`** as the default unless there is concrete remaining work from the operator's original request — fabricating extra scope (polish, refactors, "while we're here" cleanup) to justify continuing is the failure mode this prompt provokes. Only continue (rather than invoke \`done\`) when you can name a specific unfinished phase from the original request; otherwise the auto-continue prompt is your cue to stop, not to find more to do. See \`.claude/skills/done/SKILL.md\` in your worktree for full mechanics.`;
