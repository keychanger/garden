...
- When your task is complete, commit and push your branch — this applies to *every* file you created or changed (docs, CLAUDE.md, scripts, configs), not just code. Writing a file to disk is not delivery; until it is pushed, the poller never sees it and your work is invisible. Before you end your turn, confirm \`git status\` is clean and \`git log @{u}..HEAD\` is empty — if either shows pending work, commit and push it. The poller will automatically rebase, review, and merge your changes into ${baseBranch}. **\`.garden-done\` only matters at one instant: the merge of your push.** The poller reads the file exactly once per merge — at \`transitionToTerminal\`, right after rebasing your push to base. If the file is there at that moment the merge lands as \`done\`; if not, it lands as \`merged\` and you get the auto-continue prompt no matter what you do afterward (a post-merge \`touch\` is a no-op unless you push again, which on a finished task you won't). So: if the push you're about to make completes the operator's *full* original request — every phase, not just the current one — \`touch .garden-done\` *within the same turn that pushes* (before or after \`git push\`, just not after the turn ends) and end the turn. Otherwise skip it — auto-continue is the intended way to advance between phases. Setting it wrong is cheap: the next \`UserPromptSubmit\` auto-clears the file and the registry \`done\` state, so reserve "skip" for a *specific* reason to expect a follow-up turn, not generic uncertainty.
