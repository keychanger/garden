...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `trellis-picker.ts` — `⌥⇧N` hotkey picker for planting vines. Builds a `tmux display-menu` over the active project's non-retired trellises; single-active short-circuits to plant immediately; empty-state shows `[a] author` (default worker pre-seeded to invoke the trellis-author skill), `[n] scaffold` (`command-prompt` for a name → `garden trellis new`), and `[r] revive` (sub-menu of retired trellises) when retired ones exist. `buildPickerPlan` is pure so tests can assert the menu shape without driving tmux.
...
- `garden workers new <project> --workflow trellis --trellis <name> [--model opus|sonnet] [--max-iterations N]` — plant a vine bound to the named trellis. Pre-flight via `validateTrellisPlant` (refuses unknown/retired trellises; warns on missing spec sentinel or `checks` config).
- `⌥⇧N` (dashboard hotkey) — open a picker over the active project's non-retired trellises and plant immediately. One trellis: skip the menu and plant. None: empty-state with `[a] author / [n] scaffold / [r] revive` actions. The picker is the daily-driver path; the CLI is the script-friendly equivalent. `--model` overrides the trellis default (Sonnet).
...
Out of scope for v1 (deferred to v1.5+): `--override` flag for `garden trellis resume`, stagnation detection, lessons file size cap, `garden trellis budget` mid-loop cap adjustment, type-to-filter in the picker (display-menu uses arrow-key navigation only — fzf-style filtering is a v1.5 polish).
