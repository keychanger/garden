...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `judge.ts` — PreToolUse hook for `Bash`. Friction-reducer, not a gate: pre-filters plain commands (no shell metacharacters) to a zero-cost fall-through; otherwise asks Haiku via the Messages API (OAuth token from `loadCredential()`) whether to auto-approve. On `allow`, emits `permissionDecision: "allow"`. On `uncertain` (or any error/timeout/no-credential), emits `permissionDecision: "ask"` so Claude Code renders its native permission prompt in the worker pane, fires an operator alert, and flips `claudeStatus` to `"idle"` so the dashboard reflects the waiting state. The operator cycles to the worker and approves/denies. Decisions are written to `~/.garden/sessions/judge.log` and mirrored through the dashboard `log` to `dashboard.log`. `ExitPlanMode` is routed through the existing status-only `pretooluse` handler and is never seen by the judge.
