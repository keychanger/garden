...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `skills.ts` — garden-bundled Claude Code skills installed under `<worktree>/.claude/skills/`. Currently ships `done.md`: the worker invokes it when it has finished the operator's full request, and the skill writes the `.garden-done` sentinel. Skill descriptions act as Claude's trigger condition during planning, which is more reliable than instructions buried in the system prompt. Installed at worker creation (inline write inside the bootstrap script in `create.ts`) and on every refresh/bounce (via `installClaudeHooks`).
