...
- **Garden pane** (lower-left) cycles between three views: garden (bold green `garden>` prompt with auto-dispatch via zsh `command_not_found_handler`), root (general-purpose shell), and logs. `⌥[`/`⌥]` cycle when focused on the garden pane; `⌥g` jumps to garden, `⌥r` jumps to root, `⌥l` jumps to logs.
