...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `runner.ts` — `resolveGardenRunner()`: absolute command line for spawning garden's CLI from a child process. Leaf module (depends only on Node stdlib `path` + `fs`) so hooks/default.ts can import it without dragging in create.ts/poller.ts. Severs the `hooks/default.ts → create.ts → poller.ts` path of the workflows/default.ts init cycle; the companion break (extracting `handleClaudeHook` into `hook-dispatcher.ts` so `header.ts` no longer imports `workflows/index.ts`, and routing `triggerProjectPoll` through `poller-fifo.ts` rather than `poller.ts`) closes the second path. With both severed, `defaultWorkflow.hookHandlers` holds a captured value (no getter).
  - `hook-dispatcher.ts` — `handleClaudeHook(event)`: thin dispatcher that resolves the worker's workflow and routes Claude Code hook events to its `hookHandlers` methods. Extracted from `header.ts` so that `header.ts` does not statically import `workflows/index.ts` — keeping `header.ts` workflows-free is what makes the captured-value form of `defaultWorkflow.hookHandlers` safe under esbuild's bundling order.
  ...
  - `skills.ts` — garden-bundled Claude Code skills installed under `<worktree>/.claude/skills/<name>/SKILL.md` (Claude Code's required directory layout — flat `<name>.md` files are not discovered). Skill descriptions act as Claude's trigger condition during planning, which is more reliable than instructions buried in the system prompt. Installed at worker creation (inline write inside the bootstrap script in `create.ts`) and on every refresh/bounce (via `installClaudeHooks`). Currently ships two skills:
    - **`done`** — the worker invokes it when it has finished the operator's full request; writes the `.garden-done` sentinel.
    - **`handoff`** — operator-triggered: when told to pass the task to a fresh worker, the worker invokes the skill, which runs `garden handoff <target-project>` with a heredoc briefing. The CLI spawns a new named worker on the target project — a normal worker that participates in the standard review/merge/poller flow — and swaps its pane into view exactly like ⌥n would (parking the source pane under its project; for cross-project handoff also switching the active project, and the active plot if the target lives outside the current plot). The briefing is written to `~/.garden/sessions/seeds/seed-*.txt`, and a delayed `_seed-worker` subprocess polls until the new worker's `claudeStatus` leaves `loading` and then sends the seed prompt prefixed `[handoff from <source-project>/<source-worker>]`. After invoking, the source worker writes `.garden-done` per the skill recipe — the source pane is parked but its Claude is still alive long enough to write the sentinel and end the turn cleanly.
  ...
  - `header.ts` — tmux status bar: active project context (left) via `@garden_left`, build version (right) via `@garden_right`. Also renders the top-of-status-pane plot strip (`@garden_name`) with per-plot status icons (failing/asking/done/working/idle) and animates the "working" spinner by piggybacking on the status pane's SIGUSR1 tick via `~/.garden/sessions/plot-strip.template`.
  - `plot-status.ts` — aggregates worker states across a plot's projects into a single `PlotState` (`failing` > `asking` > `done` > `working` > `idle`). Drives the plot-strip icon. `merged` is folded into `working`: only `done` (worker self-declared finished via `.garden-done`) earns the green check on the strip.
  ...
  - `poller.ts` — poller coordinator: per-project event-driven dispatcher (poll, pollWorker, startProjectPoller, restartLongLivedPollers). pollWorker dispatches on `prState` through `workflow.stateHandlers` looked up from the `workflows/` registry by `entry.workflow ?? "default"` — no hard-coded switch. State-machine handler functions still live in the four sibling modules; re-exports the public surface so external imports stay stable.
  - `poller-state.ts` — state machine: transitionState (validates against the worker's `WorkflowDefinition.validTransitions`, falls back to the default workflow for unknown/legacy entries), simple terminal-state handlers (handleFailing, handleMerged, handleDone)
  - `workflows/` — workflow registry. `types.ts` defines `WorkflowDefinition` (`validTransitions` + `stateHandlers` + `hookHandlers`); `default.ts` declares the default workflow that reproduces pre-refactor behavior (validTransitions is the literal pre-refactor table, stateHandlers point at the existing handlers in poller-state/review/merge/resolve, hookHandlers points at `defaultHookHandlers` in `hooks/default.ts`); `index.ts` is the lookup (`getWorkflow(name)` warns once on unknown names and falls back to default). Design target: `WORKFLOWS.md`.
  - `hooks/default.ts` — default workflow's Claude Code hook handlers (`onSessionStart`/`onUserPromptSubmit`/`onStop`/`onNotification`/`onPreToolUse`/`onPostToolUse`). Reproduces the pre-refactor monolithic `handleClaudeHook` body. `handleClaudeHook` in `hook-dispatcher.ts` is the thin top-level dispatcher that builds a `HookContext`, looks up the worker's workflow, and calls the matching method. Hook-internal helpers (`workerFromCwd`, `readHookInput`, `routeStopHookEnd`, base-drift alert dedup) live alongside the handlers. Imports `triggerProjectPoll` from `poller-fifo.ts` directly (NOT via `poller.ts`) so the workflows/default.ts init cycle stays severed.
...
- **Auto-continue across the merge boundary** (`src/dashboard/continue.ts`, `poller.ts:maybeAutoContinue`): After a clean merge, `finalizeMerge` dispatches a delayed `_continue-worker-after-merge` subprocess that sends a "previous changes were merged, continue with the next phase" prompt to the worker pane. This removes the manual "please proceed" step in multi-phase tasks. **Worktree sync**: before the terminal-state transition, `finalizeMerge` calls `syncWorktreeToRemote` on the worker's worktree — `git fetch && git reset --hard origin/<branch>` — so the worker's HEAD matches the reviewer's force-pushed (and now-merged) tip instead of its own pre-review commits. Refuses to clobber uncommitted local edits (rare but possible if the operator typed in the pane); on dirty/fetch/reset failure raises a `poller`-source alert and the prompt itself tells the worker to sync manually. **Prompt enrichment**: `finalizeMerge` diffs `preReviewSha` (the worker's local HEAD when the reviewer launched, preserved through `merge-pending`) against the post-sync HEAD via `getChangedFilesBetween` and stashes the file list on the registry as transient `pendingContinueChangedFiles`; `continueWorkerAfterMerge` reads it, builds a preamble listing files that changed during review (so Claude re-reads them before editing instead of trusting its now-stale context), then clears the field. Empty diff (clean verdict, no rebase delta) → no preamble. The worker opts out by writing the sentinel `<worktree>/.garden-done` (`touch .garden-done` from its CWD — instructed not to commit it) before ending its turn; the worker's system prompt covers this in `src/rules.ts` `buildWorktreeRules`. Skip conditions logged at `debug`: sentinel present, `claudeStatus` is `working`/`asking` (operator already typing — same guard as interrupt-recovery), or a previous auto-continue fired within `AUTO_CONTINUE_DEBOUNCE_MS` (10s, idempotency guard against any merge-event replay). The dispatch itself logs at `info` (`auto-continued worker after merge`) so the operator sees the lifecycle transition in `⌥l` logs. Registry fields: `lastAutoContinueAt`, `pendingContinueChangedFiles`, `pendingContinueSyncFailed`. The 5s subprocess delay (longer than the 3s interrupt-recovery delay) lets postMerge and the reviewer's force-push settle before keys land in the pane. `garden pause` / `garden resume` toggle the sentinel via the registry's `worktreePath`; `killPane` does NOT need explicit cleanup because `git worktree remove --force` removes the sentinel along with the worktree.
- **Global auto-continue gate** (`src/config.ts:getAutoContinueConfig`, `poller.ts:autoContinueGateReason`): On top of the per-worker `.garden-done` opt-out, a global gate in `~/.garden/config.yml` under `autoContinue` (`enabled`, `usageThreshold`, `resumeAfterReset`) controls auto-continue across all projects. Defaults: enabled, 95% threshold, no auto-resume. The gate is consulted on every `maybeAutoContinue` after the per-worker checks. Threshold check evaluates the `5h` and `weekly` meters from the latest usage snapshot — `sonnet` is intentionally excluded since the workhorse is Opus and sonnet quota is unused. When any included meter is at or above `usageThreshold`, the gate flips `enabled=false`, persists `pausedUntil` (the latest `resetsAt` among tripped meters, so re-enabling doesn't immediately re-trip on the slower-resetting meter) and `pausedReason`, and fires a warn-level alert (source `usage`, project `garden`). With `resumeAfterReset: true`, the next call past `pausedUntil` flips `enabled` back on automatically; with it off (the default), the operator must run `garden auto on`. Manual `garden auto off` clears the paused metadata so it does not auto-resume. CLI: `garden auto [on|off|status|threshold N|resume-on-reset on|off]` (alias `auto-continue`). Skip reasons logged at `info`: `globally-disabled`, `usage-paused`, `usage-threshold`.
- **`done` is the cleanup signal; `merged` is the transient post-merge beat** (`src/dashboard/header.ts:routeStopHookEnd`, `src/dashboard/poller.ts:finalizeMerge`): the two terminal states are visually distinct in the status pane — `merged` renders neutral (just landed, mid-flow); `done` renders bold green (operator-actionable). `finalizeMerge` picks the terminal state based on `.garden-done` presence at merge time: sentinel set → `prState=done` directly (skips the transient beat); sentinel absent → `prState=merged`, which the auto-continue prompt's `UserPromptSubmit` then clears. The Stop hook covers the "operator wrote `.garden-done` after auto-continue" path: when the worker has no commits ahead of base AND `.garden-done` is present, it sets `prState=done`. Three Stop-hook end-of-turn dispositions: commits ahead → queue review (`pendingReviewAt`), no commits + `.garden-done` → set `done`, no commits + no sentinel → plain `idle`. STATUS.md invariant 4 codifies this (the spec was updated in lockstep with the code).
