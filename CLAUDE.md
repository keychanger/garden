...
- `src/dashboard/` — the dashboard. Notable modules:
  ...
  - `conversation.ts` — per-worker conversation view (`⌥c`): tail-reads the worker's Claude transcript JSONL into turns (operator prompts + verb-tagged assistant summaries) and formats the pane; transcript path captured from the hook input (`WorkerEntry.transcriptPath`) or derived from worktree cwd + session id
...
Permanent tmux layout — content is moved between visible slots and **hidden underscore-prefixed windows** (`_<project>-worker-N`, `_<project>-shell`, `_<project>-poller`, `_garden-{growhouse,root,logs,conversation}`) via `tmux swap-pane`. Pane slots are never destroyed.

- **Left column**: `garden` title pane (Claude quota meters), `status` pane (per-project worker list), and `growhouse` pane (cycles `⌥g` growhouse / `⌥r` root / `⌥l` logs / `⌥c` conversation). Title and status panes refresh via SIGUSR1, replaying pre-baked files written atomically. The `⌥c` conversation view shows the focused worker's prompt history (operator prompts + brief verb-tagged assistant summaries), read from the worker's Claude transcript JSONL by `conversation.ts` and painted via the same pre-baked-file + SIGUSR1 path; `writeConversationRendered` (`header.ts`) only parses the transcript when conversation is the active garden mode, keeping it off the hook firehose.
