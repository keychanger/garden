...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `runner.ts` — `resolveGardenRunner()`: absolute command line for spawning garden's CLI from a child process. Leaf module (depends only on Node stdlib `path` + `fs`) so hooks/default.ts can import it without dragging in create.ts/poller.ts. This severed `hooks/default.ts → create.ts → poller.ts` but not the cycle that forces `defaultWorkflow.hookHandlers` to be a getter — that cycle runs through `header.ts` (`workflows/default.ts → hooks/default.ts → header.ts → workflows/index.ts → workflows/default.ts`) and is still load-bearing.
  ...
  - `hooks/default.ts` — default workflow's Claude Code hook handlers (`onSessionStart`/`onUserPromptSubmit`/`onStop`/`onNotification`/`onPreToolUse`/`onPostToolUse`). Reproduces the pre-refactor monolithic `handleClaudeHook` body. `handleClaudeHook` in `header.ts` is now a thin dispatcher that builds a `HookContext`, looks up the worker's workflow, and calls the matching method. Hook-internal helpers (`workerFromCwd`, `readHookInput`, `routeStopHookEnd`, base-drift alert dedup) live alongside the handlers.
