...
- **Logging** (`src/dashboard/log.ts`): Structured JSON log to `~/.garden/sessions/dashboard.log`. Levels are a semantic contract, not decoration: `error` = operator must act, `warn` = possibly wrong, `info` = **lifecycle state transition** (worker moved, merge landed, alert fired), `debug` = events, heartbeats, pair-half ops, poll cycles with no transition. Default level is info; `GARDEN_LOG_LEVEL=debug` brings back the firehose. Use `log.transition(src, msg, from, to)` for state-change logs — it auto-picks info when `from !== to` and debug otherwise, so callers don't need to remember to diff before logging. Per-tool-call hooks, park/restore pair halves, and no-op poll cycles log at debug; Claude hooks that actually flip `claudeStatus` log at info.
