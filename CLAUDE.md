...
Permanent tmux layout — content is moved between visible slots and **hidden underscore-prefixed windows** (`_<project>-worker-N`, `_<project>-shell`, `_<project>-poller`, `_garden-{growhouse,root,logs,history}`) via `tmux swap-pane`. Pane slots are never destroyed.
...
- **Status**: hooks write `agentStatus`, the poller writes `prState`, `pane-died` writes `agentStatus="exited"`. Event-driven; no fallback poll. Spec: `docs/STATUS.md`.
