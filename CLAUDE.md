...
- **Right-pane clock**: the right pane (worker or project shell) shows a `%H:%M` clock in the right corner of its top border (mirroring the `garden` title in the top-left). The shared `pane-border-format` carries a right-aligned segment gated on the `@garden_clock` pane var, set on worker panes and the right-pane shell (not the left-column garden shells); tmux strftime-expands it on each `status-interval` tick — no extra process or poll.
