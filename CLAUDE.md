...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `judge.ts` — PreToolUse hook for `Bash`. Pre-filters plain commands (no shell metacharacters) to a zero-cost fall-through; otherwise asks Haiku via the Messages API (OAuth token from `loadCredential()`) whether to auto-approve. On `allow`, emits `permissionDecision: "allow"`. On `uncertain` (or any error/timeout/no-credential), emits `permissionDecision: "deny"` and fires an operator alert so the blocked command is visible in `opt-l`. The worker sees the denial reason and can adapt. Decisions are written to `~/.garden/sessions/judge.log` and mirrored through the dashboard `log` to `dashboard.log`. `ExitPlanMode` is routed through the existing status-only `pretooluse` handler and is never seen by the judge.
