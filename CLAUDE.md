...
- `src/dashboard/` — the dashboard. Notable modules:
  ...
  - `watchdog.ts` — liveness watchdog: recurring poke-only tick that recovers workers stranded in active states by a lost one-shot event
...
- **Status**: hooks write `agentStatus`, the poller writes `prState`, `pane-died` writes `agentStatus="exited"`. Event-driven; the only recurring tick is the poke-only liveness watchdog (`watchdog.ts`, `_garden-watchdog` window), which re-pokes projects whose workers sit in active states past a 5 min staleness threshold — it recovers dropped one-shot pokes and never transitions state. Spec: `docs/STATUS.md`.
