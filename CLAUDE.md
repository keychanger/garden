...
- **Worker pane clock**: worker panes show a `%H:%M` clock in the right corner of their top border (mirroring the `garden` title in the top-left). The shared `pane-border-format` carries a right-aligned segment gated on the `@garden_clock` pane var, set on worker panes only; tmux strftime-expands it on each `status-interval` tick — no extra process or poll.
