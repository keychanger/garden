...
- **Status**: hooks write `agentStatus`, the poller writes `prState`, `pane-died` writes `agentStatus="exited"`. Event-driven; the only recurring tick is the liveness watchdog (`watchdog.ts`, `_garden-watchdog` window), which re-pokes projects whose workers sit in active states past a 5 min staleness threshold and keeps each project's poller window healthy (respawning a dead one, collapsing duplicates a spawn race left) — it recovers lost event delivery and never transitions state. Spec: `docs/STATUS.md`.
