...
- **Diary**: `diary [project] [--path]` — per-project diary (free-form operator notes) opened in `$EDITOR`; `--path` prints the file path. Storage: `~/.garden/diary/<project>.md` — operator-private, outside any repo, invisible to workers and reviewers. In the dashboard, `⌥d` opens the focused project's diary in the garden pane, and switching projects (`⌥1–9` / `⌥p`) while the diary pane is active reloads it on the newly-focused project — driving nano's save+exit (`^O Enter ^X`) first so unsaved notes survive (`reloadDiaryEditor` in `navigate.ts`; nano/pico-only, since a custom `$EDITOR`'s save keys can't be driven blindly). nano/pico are launched with `-b` (word wrap) so long lines wrap to the pane width; detection is `editorIsNano` (`src/diary.ts`). (`src/diary.ts`, `src/commands/diary.ts`).
