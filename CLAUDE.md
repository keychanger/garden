...
- **Status**: hooks write `agentStatus`, the poller writes `prState`, `pane-died` writes `agentStatus="exited"`. Event-driven; the only recurring tick is the liveness watchdog (`watchdog.ts`, `_garden-watchdog` window), which re-pokes projects whose workers sit in active states past a 5 min staleness threshold and respawns poller windows that died — it recovers lost event delivery and never transitions state. Spec: `docs/STATUS.md`.
