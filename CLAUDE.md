...
- `src/dashboard/` — dashboard implementation, split by concern:
  ...
  - `runner.ts` — `resolveGardenRunner()`: absolute command line for spawning garden's CLI from a child process. Leaf module (depends only on Node stdlib `path` + `fs`) so hooks/default.ts can import it without closing the workflows/default.ts → hooks/default.ts → create.ts module-init cycle that previously forced `defaultWorkflow.hookHandlers` to be a getter.
