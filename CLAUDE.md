...
- **Project color dots**: each project's `logColor` (palette of 256-color indices in `src/log-palette.ts`; `logColorAnsi` / `logColorTmux` derive both renderings) is echoed as a small `●` beyond the logs view — after each project name in the status pane, before the active project in the bottom status bar, and before the pane label in the right pane's top border. The border dot is gated on the `@garden_color` pane var (`colour<N>`), set by `setPaneProjectColor` (`header.ts`) everywhere `@garden_clock` is applied; the format nests `#{@garden_color}` inside `#[fg=…]`, which tmux expands before parsing the style.
