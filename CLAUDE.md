...
- **Pads**: `pad [project] [--path]` — per-project scratch pad (free-form operator notes) opened in `$EDITOR`; `--path` prints the file path. Storage: `~/.garden/pads/<project>.md` — operator-private, outside any repo, invisible to workers and reviewers. In the dashboard, `⌥d` opens the focused project's pad in the garden pane (`src/pads.ts`, `src/commands/pad.ts`).
