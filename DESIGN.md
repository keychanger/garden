...
**Project color**: Every registered project carries a stable `logColor` (palette in `src/log-palette.ts`, auto-assigned by the `loadConfig()` migration), originally used to color project names in `garden logs`. The same color is echoed as a small `●` in three more places so the views agree: after each project name in the status pane (`renderQuickStatus`), before the active project name in the bottom status bar (`formatLeft`), and before the pane label in the right pane's top border. The border dot is gated on the `@garden_color` pane variable (a tmux color name like `colour208`), set by `setPaneProjectColor` (`src/dashboard/header.ts`) at the same sites that set `@garden_clock`; the shared `pane-border-format` renders `#[fg=#{@garden_color}]●#[default]` — tmux expands the nested variable before parsing the style, and the style must stay comma-free like the clock's. The palette stores 256-color indices and derives both the ANSI form (`logColorAnsi`, for pane content) and the tmux form (`logColorTmux`, for border/status-bar formats) from the same index.
