...
9. Advance the local base branch to the merged tip — `merge --ff-only` if it is the checked-out branch (also moves the working tree, so postMerge can run), otherwise `git fetch origin <base>:<base>` to advance the ref alone (working tree untouched, postMerge skipped). A clean advance is silent, as is auto-healing a redundant local commit whose clean tree already matches origin (reset onto origin, then postMerge runs). The remaining states alert only on entry: the local base diverged from origin with real local-only content (warn), the base is checked out in another worktree (warn), the on-base checkout is dirty (warn), or it is wedged/unfetchable (`stuck`, error)
