...
- **Future phases** (not yet built): token/cost capture (the Codex stderr trailer and Claude transcript `usage` fields are discarded today), and a `garden stats` read surface. The ledger records the raw facts now so the history exists when those land.
