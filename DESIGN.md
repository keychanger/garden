...
2. **pushed**: Commits detected, awaiting review launch. If Claude is actively working, reverts to working so the status label stays accurate (handleWorking gates on SHA changes, so it won't bounce back to pushed without a new commit). Once Claude is idle, launches the review.
