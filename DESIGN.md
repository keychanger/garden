...
Process status is detected via tmux's `pane_pid` and child process checks, triggered on-demand by events rather than polling. Claude Code hooks (`UserPromptSubmit`, `Stop`) installed in each worker's `.claude/settings.local.json` signal the status pane via SIGUSR1 when a worker starts or finishes processing. The status bash loop uses the hook event marker to inject a spinner on the active worker when a "prompt" event fires but pgrep hasn't yet detected tool children — this prevents the working icon from briefly flashing to idle on project switches or during thinking pauses. Lifecycle states (pushed, reviewing, merge-pending, failing, merged) come from the worker registry. Workers are displayed in aligned columns: focus indicator (filled/empty circle), process icon, name, status, and activity.
