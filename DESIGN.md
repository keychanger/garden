...
- **Future phases** (not yet built): token/cost capture (the Codex stderr trailer and Claude transcript `usage` fields are discarded today), autonomy signals (`continue.dispatched`, gate blocks, operator holds/kicks/bounces), and a `garden stats` read surface. The ledger records the raw facts now so the history exists when those land.
